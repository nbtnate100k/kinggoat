...
def parse_bulk_pipe_line(raw: str) -> tuple[str | None, str | None, str | None]:
    """Validate a pipe dump; return (full_line, bin6_from_pan, error)."""
    s = raw.strip()
    if not s:
        return None, None, None
    if "|" not in s:
        return None, None, "Need pipe format PAN|mm|yy|cvv|…"
    parts = [p.strip() for p in s.split("|")]
    if len(parts) < 4:
        return None, None, "Need at least PAN|mm|yy|cvv"
    pan = "".join(c for c in parts[0] if c.isdigit())
    if len(pan) < 13:
        return None, None, "PAN too short"
    bin6 = pan[:6]
    if len(bin6) != 6 or not bin6.isdigit():
        return None, None, "Invalid BIN from PAN"
    return "|".join(parts), bin6, None


_STOCK_CMD_HEAD_RE = re.compile(r"^/stock(?:@[A-Za-z0-9_]+)?\s*", re.IGNORECASE)


def parse_stock_bulk_from_message(text: str) -> tuple[float, dict[str, list[str]], list[str]] | None:
    """
    ``/stock $price`` + one or more pipe lines (same message or following lines).
    Returns (price, lines_by_bin6, parse_errors) or None if not this format.
    """
    ...
    if not lines:
        return None
    first = lines[0].strip()
    if not first.lower().startswith("/stock"):
        return None
    rest0 = _STOCK_CMD_HEAD_RE.sub("", lines[0], count=1).strip()
    chunks: list[str] = []
    if rest0:
        chunks.append(rest0)
    for ln in lines[1:]:
    ...
    head = chunks[0]
    low = head.lower()
    if low.startswith("bin ") or low == "bin":
        return None
    bulk_hint = "$" in head or "|" in head or len(chunks) > 1
    if not bulk_hint:
        return None
    toks = head.split(None, 1)
    praw = toks[0].lstrip("$").replace(",", "").strip()
    try:
        price = float(praw)
    except ValueError:
        return None
    ...
    raw_cards: list[str] = []
    if rest:
        raw_cards.append(rest)
    raw_cards.extend(chunks[1:])
    by_bin: dict[str, list[str]] = {}
    errors: list[str] = []
    for i, raw in enumerate(raw_cards, 1):
        line, bin6, err = parse_bulk_pipe_line(raw)
        if err:
            errors.append(f"Line {i}: {err}")
        elif line and bin6:
            by_bin.setdefault(bin6, []).append(line)
    return price, by_bin, errors
...
def admin_bulk_stock_confirm_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ Live pile", callback_data="adm_bulkpile:live"
                ),
                InlineKeyboardButton(
                    "📋 Unchecked pile",
                    callback_data="adm_bulkpile:unchecked",
                ),
            ],
            [
                InlineKeyboardButton(
                    "❌ Cancel import", callback_data="adm_bulk_cancel"
                )
            ],
        ]
    )
...
def bulk_stock_confirmation_html(
    price: float, by_bin: dict[str, list[str]], errors: list[str]
) -> str:
    tot = sum(len(v) for v in by_bin.values())
    bins_n = len(by_bin)
    parts: list[str] = [
        f"📦 <b>Bulk import</b> · {fmt_usd(price)} per line\n\n",
        f"Detected <b>{bins_n}</b> BIN bucket(s) · <b>{tot}</b> valid line(s).\n\n",
    ]
    for bk in sorted(by_bin.keys()):
        parts.append(f"· <code>{html.escape(bk)}</code> × <b>{len(by_bin[bk])}</b>\n")
    parts.append("\n<i>Tap a pile to add everything to the shop.</i>")
    if errors:
        parts.append("\n\n⚠️ <b>Skipped lines</b>\n")
        for err in errors[:12]:
            parts.append(f"{html.escape(err)}\n")
        if len(errors) > 12:
            parts.append(f"<i>… +{len(errors) - 12} more</i>\n")
    return "".join(parts)
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("adm_bulkpile:"):
        if not is_admin(user.id):
            await query.answer("Not authorized.", show_alert=True)
            return
        ...
        if pile not in VALID_PILES:
            ...
            return
        pending = context.user_data.pop("pending_bulk_stock", None)
        if not pending:
            await query.answer("Nothing to import.", show_alert=True)
            return
        by_bin: dict[str, list[str]] = pending.get("by_bin") or {}
        price = float(pending.get("price") or 0.0)
        if not by_bin:
            await query.answer("Empty batch.", show_alert=True)
            return
        load_bin_stock()
        sections_map = bin_sections_all()
        lines_added = 0
        summaries: list[str] = []
        for bin6 in sorted(by_bin.keys()):
            ln_block = list(by_bin[bin6])
            sid = f"{bin6}:{pile}"
            ...
            sec["lines"].extend(ln_block)
            sort_stock_lines(sec["lines"])
            lines_added += len(ln_block)
            summaries.append(
                f"<code>{html.escape(sid)}</code> · +{len(ln_block)} → "
                f"<b>{len(sec['lines'])}</b> total"
            )
        save_bin_stock()
        await query.answer("Stock added!")
        pile_u = pile.upper()
        await query.message.reply_text(
            "✅ <b>Bulk stock saved</b> · "
            f"<b>{pile_u}</b> pile · "
            f"{fmt_usd(price)}/line · <b>{lines_added}</b> line(s)\n\n"
            + "\n".join(summaries),
            parse_mode="HTML",
        )
        return

    if data == "adm_bulk_cancel":
        if not is_admin(user.id):
            await query.answer("Not authorized.", show_alert=True)
            return
        context.user_data.pop("pending_bulk_stock", None)
        await query.answer("Cancelled.")
        await query.message.reply_text("Bulk import cancelled.")
        return
    ...
    if data == "adm_port_upload":
        ...
        await query.message.reply_text(
            "📤 <b>Upload cards</b>\n\n"
            "<b>Fast bulk:</b>\n"
            "<code>/stock $0.35</code> then paste dumps (different BINs in one paste OK). "
            "The bot groups by PAN BIN — tap <b>Live pile</b> or <b>Unchecked pile</b> "
            "to publish immediately.\n\n"
            "<b>Manual wizard:</b> tap <b>📦 Add stock</b> "
            "(BIN → pile → price → tag → paste → <code>/done</code>). "
            "Telegram sometimes splits huge pastes — send every chunk in order "
            "(nothing commits until <code>/done</code> there).\n"
...
async def cmd_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid in _admin_stock_wizard_user_ids or context.user_data.get(
        "pending_bulk_stock"
    ):
        clear_admin_stock_flow(context, uid)
        parts.append("Stock upload cancelled.")
    if parts:
...
async def cmd_cancelstock(
    ...
    if not is_admin(uid):
        ...
        return
    if (
        uid not in _admin_stock_wizard_user_ids
        and not context.user_data.get("pending_bulk_stock")
    ):
        await update.message.reply_text(
            "Nothing to cancel — no stock wizard or bulk import pending."
        )
        return
    clear_admin_stock_flow(context, uid)
...
async def cmd_stock(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Admin: bulk pipe import, guided wizard, or one ``bin`` quick line."""
    if not update.message or not update.effective_user:
        return
    uid = update.effective_user.id
    if not is_admin(uid):
    ...
    parsed_bulk = parse_stock_bulk_from_message(update.message.text or "")
    if parsed_bulk is not None:
        price_b, by_bin, errs = parsed_bulk
        if not by_bin:
            await update.message.reply_text(
                "Paste at least one full pipe line (<code>PAN|mm|yy|cvv|…</code>) "
                "under the price, for example:\n\n"
                "<code>/stock $0.35\n372742048894932|08|25|6084|Victor|…</code>",
                parse_mode="HTML",
            )
            return
        clear_admin_stock_flow(context, uid)
        context.user_data["pending_bulk_stock"] = {
            "price": price_b,
            "by_bin": by_bin,
            "errors": errs,
        }
        await update.message.reply_text(
            bulk_stock_confirmation_html(price_b, by_bin, errs),
            parse_mode="HTML",
            reply_markup=admin_bulk_stock_confirm_keyboard(),
        )
        return
    ...
    if args[0].lower() != "bin":
        await update.message.reply_text(
            "Usage (admin):\n\n"
            "<b>Bulk (auto BIN detection)</b>\n"
            "<code>/stock $0.35</code>\n"
            "+ paste one or more dump lines (different BINs OK). "
            "Tap <b>Live</b> / <b>Unchecked</b>.\n\n"
            "<b>Wizard</b> — <code>/stock</code>\n"
            "BIN → pile → price → tag → paste → <code>/done</code>\n\n"
            "<code>/stock bin BIN $price PAN MM/YY CVV</code>\n"
            "<i>Quick one-liner → LIVE pile.</i>\n",
            parse_mode="HTML",
