"""
Pluxo — Telegram bot UI (hub, profile, top-up, payments, cart).
"""
...
⚜️ Have a good day with PluxoV2 CCs ⚜️"""
...
PLUXO_CHANNEL_URL = "https://t.me/+_OprIQ6fSSMxZWQx"
...
# section_bin (6 digits) → { price_usd, lines, listing_notes }
...
def _merge_section_into(
    ...
    """Merge BIN section payload into bucket (combine lines when same BIN exists)."""
    if bk not in bucket:
        bucket[bk] = {
        ...
        sort_stock_lines(bucket[bk]["lines"])
        return
    dest = bucket[bk]
    dest["price_usd"] = max(
        ...
        if nested:
            ...
            for key in sorted(raw.keys()):
                ...
                for k, v in sub.items():
                    ...
                    if sec:
                        _merge_section_into(merged, bk, sec)
            log.info(
...
def admin_stock_wizard_step1_intro() -> str:
    return (
        ...
        "Send the <b>6-digit BIN</b> for this listing section "
        "(e.g. <code>535585</code>). <i>One pasted line later = one sale.</i>"
    )
...
def format_my_orders_html(user_id: int, limit: int = 12) -> str:
    ...
    for i, o in enumerate(recent, 1):
        ...
        sec = html.escape(str(o.get("section_bin", ""))[:6])
...
def format_start_caption(user_id: int) -> str:
    ...
    return (
        "Welcome to <b>Pluxo</b> — grand release\n\n"
...
def main_reply_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    """Persistent bottom menu (ReplyKeyboard) — matches Pluxo layout."""
    u = ensure_user(user_id)
    total = user_shop_balance(u)
    if total == int(total):
        cred = str(int(total))
...
def section_catalog_html_title(section_bin: str) -> str:
    load_bin_stock()
    sec = bin_sections_all().get(section_bin) or {}
    price = float(sec.get("price_usd", 0.0))
    return f"{html.escape(section_bin)} · {fmt_usd(price)}"


def buy_menu_keyboard() -> InlineKeyboardMarkup:
    """One row per BIN section (shared inventory)."""
    load_bin_stock()
    rows: list[list[InlineKeyboardButton]] = []
    sections = bin_sections_all()
    for bk in sorted(sections.keys()):
        sec = sections[bk]
        n = len(sec.get("lines") or [])
        price = float(sec.get("price_usd", 0.0))
...
def section_catalog_text_and_keyboard(
    ...
    u = ensure_user(user_id)
    section_bin = section_bin.strip()[:6]
    sec = bin_sections_all().get(section_bin)
...
async def handle_buy_product(
    ...
    if not lines_flat:
        del sections_map[section_bin]
    else:
        sec["lines"] = lines_flat
    save_bin_stock()
    try:
        record_purchase_order(user.id, "shop", section_bin, bucket_key, price, line)
    except Exception:
    ...
    await query.message.reply_text(
        "✅ <b>Delivered</b> · "
        f"{section_catalog_html_title(section_bin)}\n"
...
async def handle_buy_catalog_page(
    ...
    try:
        ...
        return
    load_bin_stock()
    if section_bin not in bin_sections_all() or page < 0:
        await query.answer("Invalid.", show_alert=True)
        return
    text, kb = section_catalog_text_and_keyboard(
        section_bin, user.id, page=page
    )
...
async def handle_main_reply_menu(
    ...
    if text == BTN_REPLY_BUY:
        await msg.reply_text(
            "💳 <b>Buy cards</b>\n\n"
            "Pick a <b>BIN section</b>. Each purchased line leaves stock immediately.",
    ...
    if text == BTN_REPLY_CHANNEL:
        await msg.reply_text(
            "📣 <b>Pluxo channel</b>\n\n"
            f"<a href=\"{html.escape(PLUXO_CHANNEL_URL)}\">Tap to open PluxoMain</a>",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("Open channel", url=PLUXO_CHANNEL_URL)]]
            ),
            parse_mode="HTML",
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "adm_port_upload":
        ...
        await query.answer()
        await query.message.reply_text(
            "📤 <b>Upload cards to the bot</b>\n\n"
            "Tap <b>📦 Add stock</b> for the wizard: "
            "<b>BIN → paste lines → price → notes</b> "
            "(one shared catalog — sold lines disappear automatically).\n\n"
            "Or send <code>/stock</code> with no args, or "
            "<code>/stock bin BIN $price PAN MM/YY CVV</code> for one quick line.\n\n"
    ...
    if data.startswith("open_sec:"):
        ...
        load_bin_stock()
        if len(section_bin) != 6 or section_bin not in bin_sections_all():
            await query.answer("Invalid or empty section.", show_alert=True)
            return
        text, kb = section_catalog_text_and_keyboard(
            section_bin, user.id, page=0
        )
        await edit_safe(query, text, kb)
        await query.answer()
        return
    ...
    if data in ("m_cart", "m_buy"):
        await query.message.reply_text(
            "💳 <b>Buy cards</b>\n\n"
            "Tap a BIN section — each purchased line disappears from stock instantly.",
            reply_markup=buy_menu_keyboard(),
            parse_mode="HTML",
        )
        return
...
async def handle_admin_stock_message(
    ...
    if step == "price":
        ...
        await msg.reply_text(
            ...
            "Send text, or <code>-</code> for none.",
            parse_mode="HTML",
        )
        return
    ...
    if step == "notes":
        ...
        clear_admin_stock_flow(context, user.id)
        await msg.reply_text(
            "✅ <b>Stock saved</b> · section "
            f"<code>{html.escape(bin_sec)}</code>\n"
            f"{fmt_usd(price)} per line · <b>{len(pending)}</b> line(s) added · "
            f"<b>{n}</b> total line(s).",
            parse_mode="HTML",
        )
        return
...
async def cmd_stock(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if args[0].lower() != "bin":
        await update.message.reply_text(
            "Usage (admin):\n"
            "<code>/stock</code> — guided add (BIN → lines → price → notes)\n"
            "<code>/stock bin [legacy_slug] BIN $price PAN MM/YY CVV</code>\n\n"
            "<i>Optional <code>legacy_slug</code> (old “base” names) is ignored.</i>",
            parse_mode="HTML",
        )
        return
